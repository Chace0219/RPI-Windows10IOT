    ...
    public sealed class Mfrc522
    {
        ...
        internal async Task InitIO()
        {
            ...
            try
            {
                ...
                var dis = await DeviceInformation.FindAllAsync(spiDeviceSelector);            /* Find the SPI bus controller devices with our selector string             */
                //IReadOnlyList<DeviceInformation> devices = await DeviceInformation.FindAllAsync(spiDeviceSelector);

                _spi = await SpiDevice.FromIdAsync(dis[0].Id, settings);
